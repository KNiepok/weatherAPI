// Code generated by mockery v1.0.0. DO NOT EDIT.

package mocks

import mock "github.com/stretchr/testify/mock"
import weatherapi "github.com/kniepok/weatherAPI"

// WeatherProvider is an autogenerated mock type for the WeatherProvider type
type WeatherProvider struct {
	mock.Mock
}

// FetchWeather provides a mock function with given fields: _a0
func (_m *WeatherProvider) FetchWeather(_a0 *weatherapi.Location) (*weatherapi.Weather, error) {
	ret := _m.Called(_a0)

	var r0 *weatherapi.Weather
	if rf, ok := ret.Get(0).(func(*weatherapi.Location) *weatherapi.Weather); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*weatherapi.Weather)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(*weatherapi.Location) error); ok {
		r1 = rf(_a0)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}
